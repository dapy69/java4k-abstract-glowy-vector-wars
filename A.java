...
public class A extends Applet implements Runnable {
		...
		while (true) {
				...
				if (!paused) {
					...
					while (acc2 >= 16666667) {
						...
						// if player presses mouse button 1, accelerate in direction of the mouse pointer
							...
							ratio = PLAYER_ACC / Math.sqrt(x*x + y*y);
							player[DX2] = x * ratio;
							player[DY2] = y * ratio;
						}
						...
						if (keys[2] && fireTimer <= 0) {
							...
							bullet[X] = player[X];
							bullet[Y] = player[Y];
							bullet[DX] = PLAYER_SHOT_SPEED * Math.cos(player[FACING]);
							bullet[DY] = PLAYER_SHOT_SPEED * Math.sin(player[FACING]);
							bullet[FACING] = Math.atan2(bullet[DY], bullet[DX]);
							bullets.add(bullet);
						}
						...
						for (i = 0; i < enemyTypes.length; i++) {
							...
							if (enemyType[SPAWN_QUANTITY] > 0 && levelTime >= enemyType[NEXT_SPAWN]) {
								...
								enemyTypes[i][NEXT_SPAWN] = levelTime + enemyTypes[i][SPAWN_DELAY] * Math.pow(DIFFICULTY_SPAWN_FACTOR, (int)(levelTime/DIFFICULTY_INCREASE_PERIOD));
							}
						}
						...
						for (i = enemies.size()-1; i >= 0; i--) {
							...
							if (enemy[TYPE] == ENEMY_GENERATOR) {
								// rotate
								enemy[FACING] = Math.PI * 2 * (enemy[AGE]%(int)enemy[STATE]) / enemy[STATE];
								...
								if (enemyType[DODGE_ANGLE] > 0 && dist < 400 && Math.abs(theta) < enemyType[DODGE_ANGLE]) {
								...
								enemy[DX2] = enemyType[ACCELERATION] * Math.cos(enemy[TARGET_FACING]);
								enemy[DY2] = enemyType[ACCELERATION] * Math.sin(enemy[TARGET_FACING]);
							}
							...
							for (j = enemies.size()-1; j >= 0; j--) {
								...
								if ((bullet[X] - enemy[X]) * (bullet[X] - enemy[X]) + (bullet[Y] - enemy[Y]) * (bullet[Y] - enemy[Y]) < ENEMY_SIZE/2 * ENEMY_SIZE/2) {
									...
									if (enemy[TYPE] == ENEMY_GENERATOR) {
										...
										dist = Math.sqrt((player[X] - enemy[X]) * (player[X] - enemy[X]) + (player[Y] - enemy[Y]) * (player[Y] - enemy[Y]));
			...
			for (i = 0; i < 4; i++) {
				...
				for (j = 0; j < (i == 0? 1: entities.size()); j++) {
					...
					if (Math.abs(entity[X] - player[X]/2) < (SCREEN_WIDTH/2 + 50) && Math.abs(entity[Y] - player[Y]/2) < (SCREEN_HEIGHT/2 + 50)) {
						g2.setTransform(at2);
						g2.setComposite(ac);
	...
	private void bounce(double[] entity, boolean inwards) {
		double len = Math.sqrt(entity[X]*entity[X] + entity[Y]*entity[Y]);
